...
const removeGenre = async(uid, bookData) => {
    const docRef = doc(db, 'readers', uid)

    if(bookData.volumeInfo.categories > 1){
        for(let category in bookData.volumeInfo.categories){
            await updateDoc(docRef, {
                genres: arrayRemove(categories[bookData.volumeInfo.categories[category]])
            })
        }
    } else{
        await updateDoc(docRef, {
            genres: arrayRemove(categories[bookData.volumeInfo.categories[0]])
        })
    }
}
